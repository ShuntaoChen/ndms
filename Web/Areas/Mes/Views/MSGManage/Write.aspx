...
<script type="text/javascript">
     function sendInstantMessaging() {
       ...
       if ($(".jfile", navTab.getCurrentPanel()).children().size()==0) {
        ...
        if(!$form.valid()){
         ...
         $("#form",navTab.getCurrentPanel()).attr("action", "<%= Url.Action("DoWrite", "MSGManage") %>"+"?type=send");
    ...
    function savedraftInstantMessaging() {
       ...
       $("#form",navTab.getCurrentPanel()).attr("action", "<%= Url.Action("DoWrite", "MSGManage") %>"+"?type=save");
       $("#form",navTab.getCurrentPanel()).submit();
    }
...
<div class="page">
    <div class="panelBar" style="border-top:0px;">
		<ul class="toolBar">
			...
			<li><a class="save" href="javascript:;" onclick="savedraftInstantMessaging();"><span>存稿</span></a></li>
        ...
        <div class="tabs">
		    ...
		    <div class="tabsContent">
			    <div>
				    ...
				    <div layoutH="80" style="margin-left:-1px;float:right; display:block; overflow:auto; width:240px; border:solid 1px #CCC; line-height:21px; background:#fff">
				        <ul class="tree">
				    ...
				    <div id="messageWriteLeft" layoutH="80" style="margin-right:246px;border:solid 1px #CCC; background:#fff; height:100%">
					    <form method="post" id="form" action="<%= Url.Action("DoWrite", "MSGManage") %>" class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">

                            <%= Html.Hidden("Guid") %>

                            <div class="pageFormContent">
                                <div class="unit">
				                    <label style="width:80px; display:inherit;">收件人：</label>
                                    <div class="divtxt" id="Receive" style="margin-right:10px; float:left; display:inherit;width:80%;">
                                     <%string User = ViewBag.User; %>
                                        <span class='ledlist' id="title" <%= !string.IsNullOrEmpty(User)?"style='display: none;'":"" %>>请在右边列表选取收件人！</span>
                                        <% if(!string.IsNullOrEmpty(User)){
                                            string[] arr = User.Split('|');
                                             %>
                                        <span class="ledlist" title="<%=arr[2] %>"><a class="close" href="javascript:;" onclick="RemoveSpan(this)"></a><%=arr[2] %><input type="hidden" name="ledids" value="<%=arr[0] %>"></span>
                                       <%} %>
                                    </div>
                                </div>
                                ...
                                <div class="unit">
                                    ...
                                    <input name="MessageTitle" type="text" class="required" value="<%=ViewData["Title"] %>" style="width:80%" />
                                </div>
                                <div class="unit">
                                ...
                                <div class="unit">
                                    <label style="width:80px;">发送方式：</label>
                                    <label style="width:auto"><input type="checkbox" name="messageForm" value="1" checked />站内信</label>
                                    <label style="width:auto"><input type="checkbox" name="mailForm" value="2" style=" display:none;" />邮件</label>
                                   <%-- <label style="width:auto"><input type="checkbox" name="mobileForm" value="1"  />手机短信(只发送内容)</label>--%>
                                </div>
                                <div class="unit">
                                    <label style="width:80px;">内　容：</label>
                                    <textarea name="MessageContent" tools="simple" class="editor" style="width:80%;height:250px" rows="10"></textarea>
                                </div>
                            </div>
                        </form>
				    </div>
