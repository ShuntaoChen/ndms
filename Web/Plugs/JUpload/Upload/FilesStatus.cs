...
namespace Community.Web.JUpload.Upload
{
    ...
    {
        public const string HandlerPath = "/JUpload/Upload/";
        ...
        private void SetValues(string fileName, int fileLength, string fullPath)
        {
            ...
            var fileSize = ConvertBytesToMegabytes(new FileInfo(HttpContext.Current.Server.MapPath(fullPath)).Length);
            if (fileSize > 3 || !IsImage(ext))
            {
                thumbnail_url = "/Plugs/JUpload/Content/img/filetype/" + ext.Substring(1) + ".gif";
            }
        ...
        {
            return ext == ".gif" || ext == ".jpg" || ext == ".png"||ext==".bmp"||ext==".jpeg";
        }

        private string EncodeFile(string fileName)
        {
            return Convert.ToBase64String(System.IO.File.ReadAllBytes(HttpContext.Current.Server.MapPath(fileName)));
        }
