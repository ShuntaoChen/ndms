...
function initUI(_box) {
    ...
    /* 添加了.not(".l-input")，解决与LigerUI的css冲突 */
    $("input[type=text], input[type=password], textarea", $p).not(".l-input").addClass("textInput").focusClass("focus");
    $("input[readonly], textarea[readonly]", $p).not(".l-input").addClass("readonly");
    $("input[disabled=true], textarea[disabled=true]", $p).not(".l-input").addClass("disabled");
...
//YaoJ  添加自定义事件的时候复写此方法，可避免初始化冲突
function ownEvent() {
}

// 解决页面本身加载完后的一些处理
function afterDocumnetLoad() {
    ...
    //下拉框样式
    $("select[class *='select2']").select2({ allowClear: true, placeholder: "--请选择--" });
}
